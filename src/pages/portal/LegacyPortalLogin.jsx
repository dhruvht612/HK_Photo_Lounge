...
export function LegacyPortalLogin() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setLoading(false);
      setErr(
        'Sign-in timed out. Hard-refresh (Ctrl+Shift+R), run migrations 005 and 007 in Supabase SQL Editor, and use the exact email from Auth → Users.'
      );
    }, 22000);
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try {
      await login(email, password, { role: 'client' });
      navigate(from, { replace: true });
    } catch (ex) {
      const msg = ex?.message || 'Invalid credentials';
      setErr(
        msg.includes('Invalid login credentials')
          ? 'Wrong email or password. Use the exact email from your Supabase Users list.'
          : msg
      );
    } finally {
      setLoading(false);
